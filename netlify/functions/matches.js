...
const TEAM_NAME = "Shabab";
const SEASON = 2026;

let cache = { teamId: null, leagueId: null };
...
async function resolveTeamId() {
  ...
  const results = await apiGet("teams", { search: TEAM_NAME });
  const match = results.find(
    (r) => r.team.country === "Saudi-Arabia" && r.team.name.includes("Shabab")
  );
  cache.teamId = (match || results[0])?.team.id;
  if (!cache.teamId) throw new Error("Al-Shabab team not found");
  return cache.teamId;
}

async function resolveLeagueId() {
  if (cache.leagueId) return cache.leagueId;
  const results = await apiGet("leagues", { search: "Saudi Pro League" });
  const league =
    results.find((r) => r.league.type === "League") || results[0];
  cache.leagueId = league?.league.id;
  if (!cache.leagueId) throw new Error("Saudi Pro League not found");
...
async function getStandings(leagueId) {
  const raw = await apiGet("standings", { league: leagueId, season: SEASON });
...
async function buildPayload() {
  ...
  for (const f of fixturesRaw) {
    ...
    const round = f.league.round ? ` · الجولة ${f.league.round}` : "";
